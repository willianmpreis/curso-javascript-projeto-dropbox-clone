class DropBoxController
{
    ...
    initEvents() {
        ...
        this.btnRename.addEventListener('click', e => {
            let li = this.getFilesSelection()[0];
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            let path = 'https://firebasestorage.googleapis.com/v0/b/';
            this.uploadTask(event.target.files).then(responses => {
                responses.forEach(resp => {
                    path = path + resp.bucket + '/o/' + resp.fullPath;
                    this.getFirebaseRef().push().set({
                        name: resp.name,
    ...
    removeFolderTask(ref, name)
    {
        return new Promise((resolve, reject) => {
            ...
            folderRef.on('value', snapshot => {
                ...
                folderRef.off('value'); //Desligar o evento para que enquanto estiver no loop as alterações no banco 
                                        //nao chamem novamente a função                
    ...
    readFiles()
    {
        ...
        this.getFirebaseRef().on('value', snapshot => {
            ...
            snapshot.forEach(snapsshotItem => {
                let key = snapsshotItem.key;
                let data = snapsshotItem.val();
